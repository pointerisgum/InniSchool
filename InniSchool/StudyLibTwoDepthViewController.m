//
//  StudyLibTwoDepthViewController.m
//  InniSchool
//
...
@interface StudyLibTwoDepthViewController () <UISearchBarDelegate, IQActionSheetPickerViewDelegate>
@property (nonatomic, strong) NSString *str_SearchProcessCD;
@property (nonatomic, strong) UISearchBar *searchBar;
@property (nonatomic, strong) NSString *str_processCD;
@property (nonatomic, strong) NSArray *ar_List;
@property (nonatomic, strong) NSArray *ar_2DepthList;
@property (nonatomic, strong) NSArray *ar_3DepthList;
@property (nonatomic, strong) NSString *str_2DepthIdx;
@property (nonatomic, strong) NSString *str_3DepthIdx;
@property (nonatomic, weak) IBOutlet UITableView *tbv_List;
@property (nonatomic, weak) IBOutlet UIView *v_Depth2;
@property (nonatomic, weak) IBOutlet UIView *v_Depth3;
@property (nonatomic, weak) IBOutlet UIButton *btn_2Depth;
@property (nonatomic, weak) IBOutlet UIButton *btn_3Depth;
@end

@implementation StudyLibTwoDepthViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    
    self.screenName = @"학습 자료실 2뎁스";
    
    self.v_Depth2.layer.cornerRadius = 3.0f;
    self.v_Depth2.layer.borderWidth = .5f;
    self.v_Depth2.layer.borderColor = [UIColor colorWithRed:200.0f/255.0f green:200.0f/255.0f blue:200.0f/255.0f alpha:1].CGColor;

    self.v_Depth3.layer.cornerRadius = 3.0f;
    self.v_Depth3.layer.borderWidth = .5f;
    self.v_Depth3.layer.borderColor = [UIColor colorWithRed:200.0f/255.0f green:200.0f/255.0f blue:200.0f/255.0f alpha:1].CGColor;
    
    self.str_processCD = [NSString stringWithFormat:@"%@", [self.dic_Info objectForKey_YM:@"ProcessCDIdx"]];
    self.str_SearchProcessCD = self.str_processCD;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self initNavi];
    [self updateList:self.str_SearchProcessCD];
}
...
- (void)initNavi
{
    ...
    lb_Title.text = [self.dic_Info objectForKey_YM:@"CodeNM"];
...
- (void)showSearchResultView
{
    if( self.searchBar.text.length > 0 )
    {
        ...
        vc.isLibSearch = YES;
        vc.str_Depth = self.str_SearchProcessCD;
        [self.navigationController pushViewController:vc animated:YES];
    }
}

- (void)updateList:(NSString *)str_ProcessCD
{
    //ProcessCDIdx
    
    self.str_SearchProcessCD = str_ProcessCD;
    ...
    [dicM_Params setObject:str_ProcessCD forKey:@"processCD"];
...
- (void)showEmpyView
{
    NSArray *topLevelObjects = [[NSBundle mainBundle]loadNibNamed:@"LibEmpty" owner:self options:nil];
    UIView *view = [topLevelObjects objectAtIndex:0];
    view.tag = snEmptyViewTag;
    view.center = CGPointMake(view.center.x, self.tbv_List.frame.size.height / 2);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( [str_IsFinish isEqualToString:@"Y"] )
    {
        ...
        //학습일
        cell.lb_Date.text = [NSString stringWithFormat:@"%@ ~ %@", [dic objectForKey_YM:@"LearningStartDT"], [dic objectForKey_YM:@"LearningEndDT"]];
...
- (void)actionSheetPickerView:(IQActionSheetPickerView *)pickerView didSelectTitles:(NSArray *)titles
{
    switch (pickerView.tag)
    {
        case 1:
        {
            ...
            [self.btn_2Depth setTitle:titles[0] forState:UIControlStateNormal];

            self.ar_3DepthList = nil;
            [self.btn_3Depth setTitle:@"분류명" forState:UIControlStateNormal];
            
            for( NSInteger i = 0; i < self.ar_2DepthList.count; i++ )
            {
                NSDictionary *dic = self.ar_2DepthList[i];
                if( [titles[0] isEqualToString:[dic objectForKey_YM:@"CodeNM"]] )
                {
                    self.str_2DepthIdx = [NSString stringWithFormat:@"%@", [dic objectForKey_YM:@"ProcessCDIdx"]];
//                    self.str_processCD = self.str_2DepthIdx;
                    
                    self.tbv_List.contentOffset = CGPointZero;
                    
                    [self updateList:self.str_2DepthIdx];
                }
            }
        }
            break;
        ...
        case 2:
        {
            ...
            [self.btn_3Depth setTitle:titles[0] forState:UIControlStateNormal];

            for( NSInteger i = 0; i < self.ar_3DepthList.count; i++ )
            {
                NSDictionary *dic = self.ar_3DepthList[i];
                if( [titles[0] isEqualToString:[dic objectForKey_YM:@"CodeNM"]] )
                {
                    self.str_3DepthIdx = [NSString stringWithFormat:@"%@", [dic objectForKey_YM:@"ProcessCDIdx"]];
//                    self.str_processCD = self.str_3DepthIdx;
                    
                    self.tbv_List.contentOffset = CGPointZero;

                    [self updateList:self.str_3DepthIdx];
                }
            }
        }
            break;
...
- (IBAction)go2DepthPress:(id)sender
{
    NSMutableDictionary *dicM_Params = [NSMutableDictionary dictionary];
    [dicM_Params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"comBraRIdx"] forKey:@"comBraRIdx"];
    [dicM_Params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"userIdx"] forKey:@"userIdx"];
    [dicM_Params setObject:@"2" forKey:@"codeLevel"];
    [dicM_Params setObject:self.str_processCD forKey:@"processParentIdx"];
    ...
    [[WebAPI sharedData] callAsyncWebAPIBlock:@"learn/selectProcessCodeList.do"
                                    ...
                                    withBlock:^(id resulte, NSError *error) {
                                        ...
                                        if( resulte )
                                        {
                                            NSMutableArray *arM_ListName = [NSMutableArray array];
                                            self.ar_2DepthList = [NSMutableArray arrayWithArray:[resulte objectForKey:@"ProcessCodeList"]];
                                            for( NSInteger i = 0; i < self.ar_2DepthList.count; i++ )
                                            {
                                                NSDictionary *dic = self.ar_2DepthList[i];
                                                [arM_ListName addObject:[dic objectForKey_YM:@"CodeNM"]];
                                            }
                                            
                                            if( arM_ListName.count > 0 )
                                            {
                                            ...
                                            {
                                                [self.navigationController.view makeToast:@"데이터가 없습니다"];
                                            }
                                        }
                                    }];
}

- (IBAction)go3DepthPress:(id)sender
{
    if( self.str_2DepthIdx == nil || self.str_2DepthIdx.length <= 0 )
    {
        ALERT_ONE(@"과정분류1을 선택해 주세요");
        return;
    }
    ...
    [dicM_Params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"comBraRIdx"] forKey:@"comBraRIdx"];
    [dicM_Params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"userIdx"] forKey:@"userIdx"];
    [dicM_Params setObject:@"3" forKey:@"codeLevel"];
    [dicM_Params setObject:self.str_2DepthIdx forKey:@"processParentIdx"];
    ...
    [[WebAPI sharedData] callAsyncWebAPIBlock:@"learn/selectProcessCodeList.do"
                                    ...
                                    withBlock:^(id resulte, NSError *error) {
                                        ...
                                        {
                                            NSMutableArray *arM_ListName = [NSMutableArray array];
                                            self.ar_3DepthList = [NSMutableArray arrayWithArray:[resulte objectForKey:@"ProcessCodeList"]];
                                            for( NSInteger i = 0; i < self.ar_3DepthList.count; i++ )
                                            {
                                                NSDictionary *dic = self.ar_3DepthList[i];
                                                [arM_ListName addObject:[dic objectForKey_YM:@"CodeNM"]];
