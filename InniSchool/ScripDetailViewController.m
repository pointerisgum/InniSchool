...
@interface ScripDetailViewController () <UISearchBarDelegate>
@property (nonatomic, strong) UISearchBar *searchBar;
@property (nonatomic, strong) NSArray *ar_List;
@property (nonatomic, weak) IBOutlet UITableView *tbv_List;
@end

@implementation ScripDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    self.screenName = @"학습 자료실 상세";
}
...
- (void)updateList
{
    //http://211.232.113.65:7080/api/learn/selectIntegrationCourseList.do?comBraRIdx=10001&userIdx=1004&tabType=0&pageType=1&processCD=1
    ...
    [dicM_Params setObject:@"0" forKey:@"pageType"];    //0:전체 / 1:이달의 강의 / 2:자료실
    [dicM_Params setObject:@"0" forKey:@"tabType"];     //"탭 타입 / 0 : 전체 / 1 : 미수료 /2 : 수료"
    [dicM_Params setObject:@"1" forKey:@"archivingYN"];
...
- (void)showEmpyView
{
    NSArray *topLevelObjects = [[NSBundle mainBundle]loadNibNamed:@"ScripEmpty" owner:self options:nil];
...
//    if( [str_IsFinish isEqualToString:@"Y"] )
    if( 0 ) //학습자료실은 학습기간과 진도율을 보여주지 않는다
    {
        //수료면
